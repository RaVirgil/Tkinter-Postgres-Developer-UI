...
from helpers import guihelper as guihelper
...
def CustomSql(self,parent):

    textbox = tk.Text(self, height=1, width=25)
    label = ttk.Label(self, text="Insert custom sql command")
    label.pack()
    textbox.pack()
    execute = ttk.Button(self, text="Execute", command=lambda: guihelper.executeSql(self,textbox))
    output = ttk.Label(self, textvariable=self.console)
    output.pack(pady=15)
    execute.pack(pady=15)
